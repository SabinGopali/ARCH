...
export default function Updatepartnersinfo() {
  ...
  useEffect(() => {
    const fetchCompanyInfo = async () => {
      try {
        const res = await fetch(`/backend/Partners/getallPartners/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`/backend/Partners/update/${id}`, {
        method: "POST",
        headers: {
      ...
      if (!res.ok || data.success === false) {
        return setError(data.message || "Failed to update company information.");
      }
